...
func Load() Config {
	...
	config.PostgresUser = cast.ToString(getOrReturnDefault("POSTGRES_USER", "isanov"))
	config.PostgresPassword = cast.ToString(getOrReturnDefault("POSTGRES_PASSWORD", "i1saanoff"))
	config.PostgresDatabase = cast.ToString(getOrReturnDefault("POSTGRES_DATABASE", "brand_service"))
